...
def import_resume(
    *,
    ...
    text, normalized_path = extract_resume_text(resume_path=resume_path, resume_text=resume_text)
    if len(text) < 80:
    ...
    if normalized_path and (overwrite or not current.get("resume_local_path")):
        patch["resume_local_path"] = normalized_path

    updated_profile = db.update_profile(patch) if patch else current
    ...
    return {
        ...
        "extracted": extracted["extracted_skills"],
    }
