...
Order of operations:
  ...
  3. LLM         -> Cerebras fallback, writes back to cache on success.
...
from .llm import CerebrasClient
from .models import (
    DecisionSource,
    EngineConfig,
    FieldCandidate,
...
def resolve_field(
    ...
    llm: Optional[CerebrasClient] = None,
    ...
    if rule and score >= config.min_rule_confidence:
        ...
        if _non_empty(value):
            # Static `requires_review` is the floor; the dynamic resolver
            # can SKIP review when the user's profile makes the answer
            # unambiguous (e.g. work_auth for a US citizen). It can never
            # bypass a static True flag.
            needs_review = rule.review_for(profile)
